...
<!-- SELECT * FROM votes JOIN congress_members ON votes.politician_id = congress_members.id WHERE congress_members.name = "Rep. Erik Paulsen" -->
<!-- 21 votes -->
...
(SELECT COUNT (politician_id) FROM votes WHERE congress_members.id = votes.politician_id) AS votes_count
FROM congress_members ORDER BY votes_count DESC LIMIT 10 -->

<!-- 5. Sekarang buatlah sebuah daftar semua anggota Congress yang setidaknya mendapatkan beberapa vote dalam urutan dari yang paling sedikit. Dan juga jangan sertakan field-field yang memiliki tipe date. -->
<!-- SELECT congress_members.id, congress_members.name, congress_members.party, congress_members.location, congress_members.grade_1996,congress_members.grade_current, congress_members.years_in_congress, congress_members.dw1_score, 
(SELECT COUNT (politician_id) FROM votes WHERE congress_members.id = votes.politician_id) AS votes_count
FROM congress_members ORDER BY votes_count ASC LIMIT 10 -->
