...
<!-- 5. Sekarang buatlah sebuah daftar semua anggota Congress yang setidaknya mendapatkan beberapa vote dalam urutan dari yang paling sedikit. Dan juga jangan sertakan field-field yang memiliki tipe date. -->
 <!--
 SELECT name, party, location, grade_1996, grade_current, years_in_congress, dw1_score, COUNT(politician_id) AS numbers_in_vote FROM congress_members, votes
WHERE congress_members.id = votes.politician_id
...
<!--
//Anggota Congress yang mendapatkan vote terbanyak, yaitu:
SELECT congress_members.id, congress_members.name, congress_members.party,
COUNT(*) FROM congress_members
...
//Siapa saja yang memilih politisi tersebut, yaitu:
SELECT congress_members.name, voters.first_name, voters.last_name, table_votes.total_vote
FROM votes
LEFT JOIN ( SELECT count( * ) AS total_vote, votes.politician_id
FROM votes
GROUP BY votes.politician_id ) AS table_votes
ON votes.politician_id = table_votes.politician_id
...
<!--
SELECT congress_members.id AS politician_id, congress_members.name, congress_members.location, congress_members.grade_current, COUNT(votes.politician_id) AS total_votes
FROM votes JOIN congress_members ON votes.politician_id = congress_members.id
WHERE congress_members.grade_current < 9
GROUP BY votes.politician_id
ORDER BY congress_members.name;
-->

<!-- 3. Apa saja 10 negara bagian yang memiliki voters terbanyak? List semua orang yang melakukan vote di negara bagian yang paling populer. (Akan menjadi daftar yang panjang, kamu bisa gunakan hasil dari query pertama untuk menyederhanakan query berikut ini.) -->
<!--
SELECT (voters.first_name || ' ' || voters.last_name) AS voter, voters.gender, congress_members.id AS politician_id, congress_members.name, congress_members.location, COUNT(votes.id) AS total_count
...
<!--
SELECT voters.id AS voter_id, voters.first_name, voters.last_name, congress_members.id AS congress_id, congress_members.name,
...
GROUP BY voters.id, congress_members.id HAVING total_count > 1;
-->
